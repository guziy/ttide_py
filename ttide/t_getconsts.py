...
if has_const and has_sat and has_shallow:
    ...
    ncid = nopen(path.join(_base_dir, "t_constituents_const.nc"), "r", mmap=False)
    for key in ncid.variables.keys():
        _const[key] = ncid.variables[key].data
    ncid.close()

    ncid = nopen(path.join(_base_dir, "t_constituents_sat.nc"), "r", mmap=False)
    for key in ncid.variables.keys():
        _sat[key] = ncid.variables[key].data
    ncid.close()

    ncid = nopen(path.join(_base_dir, "t_constituents_shallow.nc"), "r", mmap=False)
    for key in ncid.variables.keys():
