...
def t_tide(xin, cython.floating dt=1, stime=None, lat=None,
           str out_style='classic',
           outfile=None,
           corr_fs=[0, 1e6], corr_fac=[1, 1],
           str secular='mean',
           infiname=[], infirefname=[],
           cython.floating ray=1,
           shallownames=[], constitnames=[],
           str errcalc='cboot', cython.floating synth=2,
           str lsq='best'):
    ...
    # beg dummy defines / to correct when this code is activated
    infamprat = np.zeros((1, 1))
    infph = np.zeros((1, 1))
    i = 0
    # end dummy defines
