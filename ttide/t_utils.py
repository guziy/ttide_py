...
def constituents(minres, constit, shallow, infname, infref, centraltime):
    ...
    if constit.size:
        ...
        for k in range(0, (constit.shape[0])):
            ...
            if j1.size == 0:
                print("Can't recognize name " + constit[k].decode(enc) + " for forced search")
            else:
                ju = np.append(ju, j1)
    ...
    if shallow.size != 0:
        ...
        for k in range(0, (shallow.shape[0])):
            ...
            if j1.size == 0:
                print("Can't recognize name " + shallow[k].decode(enc) + " for forced search")
            else:
                ju = np.append(ju, j1)
    ...
    if infname.size != 0:
        ...
        for k in range(1, (infname.shape[0] + 1)):
            ...
            if 0 in j1.shape:
                print("Can't recognize name" + infname[(k - 1), :] + " for inference")
            else:
                jinf[(k - 1)] = j1
    ...
    for k in range(0, fu.shape[0]):
        ...
        N = np.hstack(
            [
                ...
                np.dot(np.squeeze(Mat[:, :, l]), np.random.randn(4, nreal - 1)),
            ]
        )
        NP[(k), :] = N[0, :] + 1j * N[1, :]
        NM[(k), :] = N[2, :] + 1j * N[3, :]
...
def residual_spectrum(xres, fu, dt):
    ...
    fx, Pxr = sps.welch(np.real(xres), window=np.hanning(nx), noverlap=np.ceil(nx / 2), nfft=nx, fs=1 / dt, nperseg=nx)
    Pxr = Pxr / 2 / dt
    fx, Pxi = sps.welch(np.imag(xres), window=np.hanning(nx), noverlap=np.ceil(nx / 2), nfft=nx, fs=1 / dt, nperseg=nx)
    Pxi = Pxi / 2 / dt
    ...
    for k in range(nfband - 1, -1, -1):
        jband = np.flatnonzero(
            np.all(np.vstack([fx >= fband[(k), 0], fx <= fband[(k), 1], np.isfinite(Pxr)]).T, axis=1)
        )
        if any(jband):
...
def variance_str(out):
    ...
    outstr += ("var(data)= {:.2f}" + " " * 4 + "var(prediction)= {:.2f}" + " " * 4 + "var(residual)= {:.2f}\n").format(
        x, xp, xr
    )
    ...
    if np.iscomplexobj(out["xin"]):
        ...
        outstr += (
            "var(data)= {:.2f}" + " " * 4 + "var(prediction)= {:.2f}" + " " * 4 + "var(residual)= {:.2f}\n"
        ).format(y, yp, yr)
        outstr += "var(prediction)/var(data) (%) = {:.1f}\n\n".format(100 * yp / y)

        outstr += "total_var= {:f} pred_var=  {:f}\n".format((x + y), (xp + yp))
        outstr += "total_var/pred_var (%) =  {:.1f}  \n".format(100 * (xp + yp) / (x + y))
    return outstr
...
def classic_style(out):
    ...
    outstr += (
        ...
        % (out["nobs"], out["ngood"], np.dot(max(out["xin"].shape), out["dt"]) / 24)
    ) + "\n"
    if "stime" in out:
        outstr += "start time: {}\n".format(time.num2date(out["stime"]).strftime("%Y-%m-%d %H:%M:%S"))
    ...
    if out["isComplex"]:
        ...
        for k, fuk in enumerate(out["fu"]):
            if out["snr"][k] > out["synth"]:
                ...
                star = " "
            outstr += fmt.format(
                star=star, name=out["nameu"][k].decode(enc), fuk=fuk, c=out["tidecon"][k], snr=out["snr"][k]
            )
    else:
        outstr += "        tidal amplitude and phase with 95 % CI estimates\n"
        outstr += " tide      freq        amp      amp_err" + "   pha      pha_err    snr\n"
        fmt = "{star} {name}  {fuk:9.7f}  {c[0]:9.4f}  {c[1]:8.3f}  {c[2]:8.2f}  {c[3]:8.2f}  {snr:8.2g}\n"
        for k, fuk in enumerate(out["fu"]):
            if out["snr"][k] > out["synth"]:
                star = "*"
            else:
...
def pandas_style(out, dfTF=False):
    ...
    if out["isComplex"]:
        colnames = ["Freq", "Major", "Major Err", "Minor", "Minor Err", "Inc", "Inc Err", "Phase", "Phase Err", "SNR"]
